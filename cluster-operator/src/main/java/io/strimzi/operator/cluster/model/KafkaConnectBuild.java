...
/**
 * Model class for Kafka Connect Build - this model handled the build of the new
 * image with custom connectors
 */
public class KafkaConnectBuild extends AbstractModel {
    ...
    protected KafkaConnectBuild(Reconciliation reconciliation, HasMetadata resource,
        ...
        super(reconciliation, resource, KafkaConnectResources.buildPodName(resource.getMetadata().getName()),
                COMPONENT_TYPE, sharedEnvironmentProvider);

        this.image = System.getenv().getOrDefault(ClusterOperatorConfig.STRIMZI_DEFAULT_KANIKO_EXECUTOR_IMAGE,
                DEFAULT_KANIKO_EXECUTOR_IMAGE);
    }

    /**
     * Creates the KafkaConnectBuild instance from the Kafka Connect Custom
     * Resource.
     *
     * @param reconciliation            The reconciliation
     * @param kafkaConnect              Kafka Connect CR with the build
     *                                  configuration
    ...
    public static KafkaConnectBuild fromCrd(Reconciliation reconciliation,
        ...
        if (spec.getBuild() != null) {
            ...
            // The additionalKanikoOptions are validated separately to avoid parsing the
            // list twice
            if (spec.getBuild().getOutput() != null
                    && spec.getBuild().getOutput() instanceof DockerOutput dockerOutput) {

                // Validation to check if KafkaConnect .spec.image equals
                // .spec.build.output.image name
                if (dockerOutput.getImage() != null && spec.getImage() != null
                        && dockerOutput.getImage().equals(spec.getImage())) {
                    throw new InvalidResourceException(
                            "KafkaConnect .spec.image cannot be the same as .spec.build.output.image");
                }
            ...
            if (template.getBuildConfig() != null) {
                ...
                if (template.getBuildConfig().getMetadata() != null) {
                    ...
                    if (template.getBuildConfig().getMetadata().getAnnotations() != null) {
                        result.templateBuildConfigAnnotations = template.getBuildConfig().getMetadata()
                                .getAnnotations();
                    }
                }
            }
    ...
    private static void validateBuildConfiguration(Build build) {
        if (build.getPlugins() == null) {
            throw new InvalidResourceException(
                    "List of connector plugins is required when Kafka Connect Build is used.");
        }
        ...
        if (names.size() != build.getPlugins().size()) {
            throw new InvalidResourceException(
                    "Connector plugins names have to be unique within a single KafkaConnect resource.");
        }
    ...
    /**
     * Validates the additional Kaniko options configured by the user against the
     * list of allowed options. If any
     * options which are not allowed are found, it raises an
     * InvalidResourceException exception.
     *
     * @param desiredOptions List of additional Kaniko options configured by the
     *                       user
     */
    private static void validateAdditionalKanikoOptions(List<String> desiredOptions) {
        ...
        if (!forbiddenOptions.isEmpty()) {
            throw new InvalidResourceException(
                    ".spec.build.additionalKanikoOptions contains forbidden options: " + forbiddenOptions);
        }
    }
     ...
     *
     * @param dockerfile Instance of the KafkaConnectDockerfile class with the
     *                   prepared Dockerfile
     *
     ...
     *
     * @return Instance of the KafkaConnectDockerfile class with the prepared
     *         Dockerfile
     */
    ...
    /**
     * Generates builder Pod for building a new KafkaConnect container image with
     * additional connector plugins
     *
     * @param isOpenShift      Flag defining whether we are running on OpenShift
     * @param imagePullPolicy  Image pull policy
     * @param imagePullSecrets Image pull secrets
     * @param newBuildRevision Revision of the build which will be build used for
     *                         annotation
     *
     * @return Pod which will build the new container image
     */
    public Pod generateBuilderPod(boolean isOpenShift, ImagePullPolicy imagePullPolicy,
            List<LocalObjectReference> imagePullSecrets, String newBuildRevision) {
    ...
    private List<Volume> getVolumes(boolean isOpenShift) {
        ...
        volumes.add(
                VolumeUtils.createConfigMapVolume("dockerfile", KafkaConnectResources.dockerFileConfigMapName(cluster),
                        Collections.singletonMap("Dockerfile", "Dockerfile")));

        if (build.getOutput() instanceof DockerOutput output) {

            if (output.getPushSecret() != null) {
                volumes.add(VolumeUtils.createSecretVolume("docker-credentials", output.getPushSecret(),
                        Collections.singletonMap(".dockerconfigjson", "config.json"), isOpenShift));
            }
        } else {
    ...
    private List<VolumeMount> getVolumeMounts() {
        ...
        if (build.getOutput() instanceof DockerOutput output) {
            ...
            if (output.getPushSecret() != null) {
                volumeMounts.add(new VolumeMountBuilder().withName("docker-credentials")
    ...
    /**
     * Generates list of container environment variables for the builder pod.
     * Currently contains only the proxy env vars
    ...
    /* test */ Container createContainer(ImagePullPolicy imagePullPolicy) {
        List<String> args = additionalKanikoOptions != null ? new ArrayList<>(4 + additionalKanikoOptions.size())
                : new ArrayList<>(4);
        ...
        return ContainerUtils.createContainer(
                ...
                image,
                args,
                securityProvider.kafkaConnectBuildContainerSecurityContext(
    ...
    /**
     * Generates a BuildConfig which will be used to build new container images with
     * additional connector plugins on OCP.
     *
    ...
    /**
     * Generates OpenShift Build Request to start a new build using OpenShift Build
     * feature
     *
     * @param buildRevision The revision of the build (to indicate if rebuild is
     *                      needed)
     *
