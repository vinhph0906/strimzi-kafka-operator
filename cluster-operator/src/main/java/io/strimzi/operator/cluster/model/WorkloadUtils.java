...
/**
 * Shared methods for creating Workload resources (Deployments, StatefulSets,
 * StrimziPodSets)
 */
public class WorkloadUtils {
         ...
         * @param updateStrategy  Deployment update strategy (Recreate or Rolling
         *                        Update)
         * @param podTemplateSpec The PodTemplateSpec which defines how the pods created
         *                        by this Deployment look like
        ...
        public static Deployment createDeployment(
                ...
                return new DeploymentBuilder()
                                ...
                                .withAnnotations(Util.mergeLabelsOrAnnotations(annotations,
         ...
         * @param replicas       Number of replicas
         * @param annotations    Additional annotations which should be set on the
         *                       PodSet. This might contain annotations
         *                       for tracking storage configuration, Kafka versions and
         *                       similar.
         * @param selectorLabels Labels used for the Pod selector in the
         *                       StrimziPodSetSpec
         * @param podCreator     Function for generating the Pods which should be
         *                       included in this PodSet based on their
         *                       index number.
        ...
        public static StrimziPodSet createPodSet(
                ...
                return new StrimziPodSetBuilder()
                                ...
                                .endMetadata()
                                .withNewSpec()
                                .withSelector(new LabelSelectorBuilder().withMatchLabels(selectorLabels.toMap())
                                                .build())
                                .withPods(pods)
                                .endSpec()
                                .build();
         ...
         *                       StrimziPodSetSpec
         * @param podCreator     Function for generating the Pods which should be
         *                       included in this PodSet based on the node
         *                       reference.
         *
         * @return Created PodSet
         */
        ...
        /**
         * Creates a stateful Pod for use with StrimziPodSets. Stateful in this context
         * means that it has a stable name and
         ...
         * @param strimziPodSetName       Name of the StrimziPodSet which is used to
         *                                generate the controller labels
         ...
         * @param podAnnotations          Additional annotations used for the pod. Used
         *                                to track things such as storage
         *                                configuration, Kafka versions, configuration
         *                                or certificate hash stubs etc.
        ...
        public static Pod createStatefulPod(
                ...
                Pod pod = new PodBuilder()
                                .withNewMetadata()
                                .withName(name)
                                .withLabels(labels.withStrimziPodName(name).withStatefulSetPod(name)
                                                .withStrimziPodSetController(strimziPodSetName)
                                                .withAdditionalLabels(
                                                                Util.mergeLabelsOrAnnotations(defaultPodLabels,
                                                                                TemplateUtils.labels(template)))
                                                .toMap())
                                .withNamespace(namespace)
                                .withAnnotations(Util.mergeLabelsOrAnnotations(podAnnotations,
                                                TemplateUtils.annotations(template)))
                                .endMetadata()
                                .withNewSpec()
                                ...
                                .withVolumes(volumes)
                                .withTolerations(template != null && template.getTolerations() != null
                                                ? removeEmptyValuesFromTolerations(template.getTolerations())
                                                : null)
                                .withTerminationGracePeriodSeconds(
                                                template != null ? (long) template.getTerminationGracePeriodSeconds()
                                                                : 30L)
                                ...
                                .withSchedulerName(template != null && template.getSchedulerName() != null
                                                ? template.getSchedulerName()
                                                : "default-scheduler")
                                .withHostAliases(template != null ? template.getHostAliases() : null)
                                .withTopologySpreadConstraints(
                                                template != null ? template.getTopologySpreadConstraints() : null)
                                .withNodeSelector(nodeSelector)
                                .endSpec()
                                .build();

                // Set the pod revision annotation
                pod.getMetadata().getAnnotations().put(PodRevision.STRIMZI_REVISION_ANNOTATION,
                                PodRevision.getRevision(reconciliation, pod));
         ...
         *
         * @param workloadName            Name of the workload resource which will own
         *                                this Pod (Deployment or StatefulSet)
         * @param labels                  Labels of the Pod
         * @param template                Pod template with custom configurations
         * @param defaultPodLabels        The default pod labels
         * @param podAnnotations          Additional annotations used for the pod. Used
        ...
        /**
         * Creates a Pod which can be used as a standalone pod to for example execute
         * some task.
         *
         * @param name                    Name of the Pod
         * @param namespace               Namespace of the Pod
        ...
        public static DeploymentStrategy deploymentStrategy(
                        io.strimzi.api.kafka.model.common.template.DeploymentStrategy strategy) {
        ...
        /**
         * If the toleration value is an empty string, set it to null. That solves an
         * issue when built STS contains a filed
         * with an empty property value. K8s is removing properties like this, and thus
         * we cannot fetch an equal STS which was
         * created with (some) empty value.
         *
         * @param tolerations Tolerations list to check whether toleration value is an
         *                    empty string and eventually replace it by null
         *
        ...
        public static List<Toleration> removeEmptyValuesFromTolerations(List<Toleration> tolerations) {
                if (tolerations != null) {
                        tolerations.stream().filter(
                                        toleration -> toleration.getValue() != null && toleration.getValue().isEmpty())
                                        .forEach(emptyValTol -> emptyValTol.setValue(null));
                        return tolerations;
                } else {
                        return null;
                }
         ...
         *
         * @param template     Pod template which maybe contains custom image pull
         *                     secrets configuration
         * @param defaultValue The default value which should be used if the image pull
         *                     secrets are not set
         *
         * @return Custom list of image pull secrets or default value if not defined
         */
        /* test */ static List<LocalObjectReference> imagePullSecrets(PodTemplate template,
                        List<LocalObjectReference> defaultValue) {
                return template != null && template.getImagePullSecrets() != null ? template.getImagePullSecrets()
                                : defaultValue;
        }
}