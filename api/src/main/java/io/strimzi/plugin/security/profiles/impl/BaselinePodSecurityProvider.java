...
/**
 * The default implementation of the PodSecurityProvider. It implements the
 * Baseline Kubernetes security profile.
 */
public class BaselinePodSecurityProvider implements PodSecurityProvider {
    ...
    /**
     * Internal method which checks whether persistent storage (either Persistent
     * Claim storage or a JBOD storage with
    ...
    /**
     * Internal method whichcreates a Pod security context for Pods using persistent
     * storage (Kafka and ZooKeeper). If
     * any user-supplied pod security context is set, it will be used. Otherwise:
     * - if running on OpenShift, no context will be set as OpenShift injects its
     * own context
     * - if running outside of OpenShift, the fsGroup will be set to the group ID 0
     * which is the default in the containers
     *
